...
# Static sound player
static = vlc.MediaPlayer("media/static.wav")
static.audio_set_volume(0)
static.play()
benny = vlc.MediaPlayer("media/benny.mp3")
benny.audio_set_volume(100)
benny.play()

def tuningup():
    if tunedown_queue.empty():
        tuneup_queue.put(1)
        curvol = benny.audio_get_volume()
        newvol = curvol - 10
        benny.audio_set_volume(newvol)
        statvol = static.audio_get_volume()
        newstatvol = statvol + 10
        static.audio_set_volume(newstatvol)
    else:
        tunedown_queue.get()
